...
for (holiday in holidays){
  formula <- as.formula(paste(single_rate, "as.integer(Pre) + as.integer(relative_date)", sep = "~"))   ## this and the next line is what you change to use a different model 
  model <- lm(formula, data = holiday)       ### 
  estimate_pre <- summary(model)$coef["as.integer(Pre)", "Estimate"]   ## change this line and next two lines to add different data to the table 
  p_value_pre <- summary(model)$coef["as.integer(Pre)", "Pr(>|t|)"]
  estimate_date <- summary(model)$coef["as.integer(relative_date)", "Estimate"]  
  p_value_date <- summary(model)$coef["as.integer(relative_date)", "Pr(>|t|)"]
  r_squared <-summary(model)$adj.r.squared
  a_rate_value <- rbind(a_rate_value, list(estimate_pre, p_value_pre, estimate_date, p_value_date, r_squared, summary(model)))
}
...
for (rate in centered_rates){
  new_sublist <- list(paste("Pre", rate, sep = " "), paste("p value Pre", rate, sep = " "), paste("relatve_date", rate, sep = " "), paste("p value relative_date", rate, sep = " "), paste(rate, "Adj Rsq", sep = " "), paste(rate, "Summary", sep = " ") )
